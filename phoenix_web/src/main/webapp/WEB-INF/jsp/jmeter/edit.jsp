...
<table class="table table-bordered table-hover definewidth m10">
    ...
    <tr>
     	<td class="tableleft">请求实现类型：</td>
        <td><input type="radio" checked="checked" value="HttpClient4" name="requestImpl">HttpClient&nbsp;&nbsp;&nbsp;&nbsp;
        <input type="radio" <c:if test="${perfBean.requestImpl eq 'Java'}">checked="checked"</c:if> value="Java" name="requestImpl">Java&nbsp;&nbsp;&nbsp;&nbsp;post方式发送body数据时，建议选择Java</td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td>
            <input type="checkbox" id="useBodyString" name="useBodyString" <c:if test="${perfBean.useBodyString eq 'on'}"> <% out.print("checked='checked'"); %> </c:if>  onclick="isShow(this.name)" />选中代表body参数将随url一起发送，如果body参数为json，建议在header中添加:context-type->application/json，使用body参数后，被测url将不能参数化
            <br>
            <div id="isuseBodyStringDiv" style="display: none">
	            <div class="table1">
       				 <textarea style="width:98%;height:60px" id="bodyString" name="bodyString">${perfBean.bodyString}</textarea>
	        	</div>
        	</div>
        </td>
    </tr>
