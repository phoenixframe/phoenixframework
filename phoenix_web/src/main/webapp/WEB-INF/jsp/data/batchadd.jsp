...
<table class="table table-bordered table-hover definewidth m10" id="OwnershipStructure">
		<tr>
		<td colspan="2"><input name="caseId" id="caseId" type="hidden" value="${caseBean.id }"/>所属用例：${caseBean.caseName } </td>
		</tr>
				<tr>
					<td>数据标识</td>
					<td>数据内容</td>
			...
			<tr id="StructureRight">
				<td><input type="text" name="caseDataBeanList[0].dataName"/></td>
				<td><input type="text" name="caseDataBeanList[0].dataContent"/><input name="caseDataBeanList[0].caseBean.id" type="hidden" value="${caseBean.id }"/>
					<input id="btnAddRow" class="rowOperBtn" onclick="AddStructureRow()" type="button" value=" + " /> 
				</td>
			</tr>
			<tr>
				<td colspan="2"><button type="submit" class="btn btn-primary">提交</button>&nbsp;&nbsp;&nbsp;<button type="button" class="btn btn-success" name="backid" id="backid">返回列表</button></td>
			</tr>
</table>
</form>
<script type="text/javascript">
    $(function () {       
		$('#backid').click(function(){
			...
			window.location.href=basePath+"/data/list/"+caseId;
		 });
	...
	function AddStructureRow() {
		...
		td0.innerHTML = "<input name='caseDataBeanList["+index+"].dataName' type='text'/> ";
		
		var td1 = tr.insertCell(1);
		td1.setAttribute("align", "left");
		td1.innerHTML = "<input name='caseDataBeanList["+index+"].dataContent' type='text'/> <input name='caseDataBeanList["+index+"].caseBean.id' type='hidden' value='${caseBean.id }'/>&nbsp;&nbsp;&nbsp;<input id='btnDelRow' class='rowOperBtn' type='button' value=' - ' onclick='DelStructureRow("+index+")'/>";
	}
